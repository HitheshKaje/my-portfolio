      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="absolute top-full left-0 w-full glass-panel border-t-0 rounded-t-none flex flex-col items-center py-8 gap-6 md:hidden"
          >
            {navLinks.map((link) => (
              <a
                key={link.name}
                ...
                className="text-slate-900 dark:text-white hover:text-secondary dark:hover:text-secondary font-orbitron font-medium text-lg uppercase tracking-wider"
              >
                {link.name}
              </a>
            ))}
